...
def token_count(model: Path, prompt: str) -> int:
    output = checked([str(BINARY), "-m", str(model), "-p", prompt, "--tokenize-only"], 30)
...
def state_context(path: Path) -> tuple[int, int]:
    ...
    if magic != b"MBLKSTAT" or version != 3 or (layers, rank, rope) != (61, 512, 64):
...
def run(args: argparse.Namespace) -> int:
    ...
    if not args.prompt or not 1 <= args.max_output_tokens <= 10_000:
        raise Error("prompt must be nonempty and output tokens must be in [1,10000]")
    if not 64 <= args.context <= 65_536 or not 30 <= args.stall_timeout <= 3_600:
        raise Error("context must be [64,65536] and stall timeout [30,3600] seconds")
    ...
    inputs = token_count(args.model, args.prompt)
    ...
    command = [
        str(BINARY), "-m", str(args.model), "-p", args.prompt,
